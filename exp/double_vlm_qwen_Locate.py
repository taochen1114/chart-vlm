...
def run_vlm_multi_images(matched_pages, question, vlm_answer_fn, prompt_type, resize=(720, 720)):
    ...
    images = [Image.open(p['image_path']).resize(resize) for p in matched_pages]
    prompt = Prompt[prompt_type] + f"\n\n這些圖片可能包含答案，請找出所有符合的頁碼，並在 page 欄位中列出。\n\n{bginfo}"
    prompt = f"{question}\n\n{prompt}"
    ...
    print(f"📄 多頁推論輸出：{repr(output)}")
    return [{"page": p["page"], "answer": output} for p in matched_pages]
...
if __name__ == '__main__':
    ...
    for idx, q in enumerate(questions, 1):
        ...
        if prompt_type == "Locate":
            res = run_vlm_multi_images(pages, q, vlm_answer, prompt_type="Locate")
        else:
            res = run_vlm(pages, q, vlm_answer, prompt_type=prompt_type)
